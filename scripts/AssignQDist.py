...
parser.add_option('-r',dest='dist_out',default='./Assignments.h5.QDists', help='Distance to generator output filename ( Can NOT exist now ). [ ./Assignments.h5.QDists ]')
...
Gens = load( options.gens_FN ).astype(int)
PrepGens = DistanceMetric.QData( Gens )

QDist = DistanceMetric.QMetric()
def AssignTraj( trajFN ):
    print " Assigning trajectory: %s " % trajFN
    trj = load( trajFN ).astype( int )
    PrepTrj = DistanceMetric.QData( trj )
    N = len( trj )
    Ass1d = []
    Qdist1d = []
    for i in xrange( N ):
        Dists = QDist.GetMultiDistance( PrepGens, PrepTrj, i )
        Ass1d.append( Dists.argmax() )
        Qdist1d.append( Dists.max() )
