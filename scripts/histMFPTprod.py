...
parser.add_option('--divisor',dest='nsDiv',type=float,default=1.,help='Number to divide the values by in order to convert to ns [ 1 ]')
parser.add_option('-o',dest='out_FN',default='MFPT_hists.pdf',help='Output filename [ MFPT_hists.pdf ]')
parser.add_option('-t',dest='titles',default='A,B,C',help='Titles for the plots, should be separated by commas [ A,B,C ]')
parser.add_option('-u',dest='units',default='ns',help='Units to place in x-label [ ns ]')
...
figure(figsize=(15,5) )
Dats = [ UF, FU, UU ]
Dats = [ d / options.nsDiv for d in Dats ]
#Titles = [ 'U to F', 'F to U', 'U to U']
...
for i in range(1,4):
   ...
   xscale('log')
   title( Titles[i-1] )
   xlabel('MFPT (%s)' % options.units)
   yticks([])
subplot(131)
ylabel('Relative Frequency (Linear Scale)')
#subplot(221)
