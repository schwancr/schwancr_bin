...
# 2) The radius to test points from the interior (radius)
...
def main():
  ...
  if sys.argv[1] == 'help' or sys.argv[1] == '-h':
    ...
    print " INPUT:\n\t1) Filename of .gro file (groFile)" 
    print " \t2) The radius to test points from the interior (radius)"
...
#      outTopFile.write('#ifdef DSTRST\n#include "dstrst_'+matchObj.group(1)+'"\n#endif\n')
 ...
 #           continue
          if atomCounted in keepAtms:
            outputStr = '{0:{align}{fill}5}     1  1000  1000  1000\n'.format(atomNum,align='<',fill=' ')
