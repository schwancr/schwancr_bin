...
parser.add_option('--rr',dest="res_ranges",action='append',help='Residue range to test contactsn Number of arguments should be even.\n'+
...
def getContacts():
   ...
   if len( options.res_ranges ) != 2:
      print "You need to input two ranges with --rr"
      exit()

   sets = [ thing.split(',') for thing in options.res_ranges ]
   setA, setB = [ range( int(thing[0]), int(thing[1]) + 1 ) for thing in sets ]
# First parse the contact file:
   ...
   for line in lines[1:]:
      ...
      atomA, atomB = int( line.split()[1] ), int( line.split()[3] ) 
      if atomA in setA:
         if atomB in setB:
            contactList.append( ( atomA, atomB ) )
      elif atomA in setB:
         if atomB in setA:
            contactList.append( ( atomA, atomB ) )
      else:
