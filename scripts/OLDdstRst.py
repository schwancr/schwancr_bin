...
# 2) Beginning atom index for interior piece (beginAtm)
# 3) End atom index for interior piece (endAtm)
# 4) The radius to test points from the interior (radius)
# 5) Filename of .top file (topFile)
...
def main():
  ...
  if sys.argv[1] == 'help' or sys.argv[1] == '-h':
    print "   This script reads in 5 input arguments and outputs a number of files"
    print " INPUT:\n\t1) Filename of .gro file (groFile)" 
    print " \t2) Beginning atom index for interior piece (beginAtm)"
    print " \t3) End atom index for interior piece (endAtm)"  
    print " \t4) The radius to test points from the interior (radius)"
    print " \t5) Filename of .top file (topFile)"
    ...
    print "\n  Additionally, the script creates a new topFile dstrst_...top"
  ...
  beginAtm = int(sys.argv[2])
  endAtm = int(sys.argv[3])
  radius = float(sys.argv[4])
  topFile = open(sys.argv[5],'r')
  ...
  outTopFile = open('restrained_'+sys.argv[5],'w')
